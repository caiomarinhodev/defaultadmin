function Game() {
    ...
    this.next = function () {
        if (!p.human && p.money < 0) {
            ...
            if (p.money < 0) {
                popup("<p>" + p.name + " está falido. Todos os seus ativos serão transferidos para " +
                    player[p.creditor].name + ".</p>", game.bankruptcy);
            } else {
                roll();
            }
    ...
    this.auction = function () {
        ...
        popup("<div style='font-weight: bold; font-size: 16px; margin-bottom: 10px;'>Leilao <span id='propertyname'></span></div><div>Lance mais alto = $<span id='highestbid'></span> (<span id='highestbidder'></span>)</div><div><span id='currentbidder'></span>, é sua vez de fazer lances.</div<div><input id='bid' title='Insira um valor para ofertar " + s.name + ".' style='width: 291px;' /></div><div><input type='button' value='Licitar' onclick='game.auctionBid();' title='Faça seu lance' /><input type='button' value='Passar' title='Ignorar os lances desta vez.' onclick='game.auctionPass();' /><input type='button' value='Sair do leilao' title='Parar de ofertar em " + s.name + " completamente.' onclick='if (confirm(\"Tem certeza de que deseja parar completamente de licitar nesta propriedade?\")) game.auctionExit();' /></div>", "blank");
            ...
            if (currentbidder == highestbidder) {
                ...
                if (!p.human) {
                    ...
                    if (bid === -1 || highestbid >= p.money) {
                        ...
                        window.alert(p.name + " saiu do leilao.");
                        continue;
                        ...
                        this.auctionBid(bid);
                        window.alert(p.name + " ofertou $" + bid + ".");
                        continue;
                    }
                    return;
                } else {
    ...
    this.auctionBid = function (bid) {
        ...
        if (bid === "" || bid === null) {
            document.getElementById("bid").value = "Por favor, entre com uma oferta.";
            document.getElementById("bid").style.color = "red";
        } else if (isNaN(bid)) {
            document.getElementById("bid").value = "Sua oferta deve ser um numero.";
            document.getElementById("bid").style.color = "red";
        } else {

            if (bid > player[currentbidder].money) {
                document.getElementById("bid").value = "Voce nao tem dinheiro suficiente para ofertar $" + bid + ".";
                document.getElementById("bid").style.color = "red";
                ...
                document.getElementById("bid").value = "Seu lance deve ser mais alto que o lance mais alto ($" + highestbid + ")";
                document.getElementById("bid").style.color = "red";
            }
        }
    };
    ...
    var tradeMoneyOnChange = function (e) {
        ...
        if (isNaN(amount)) {
            this.value = "O valor deve ser um numero.";
        ...
        if (amount < 0) {
            this.value = "O valor deve ser maior que zero.";
            this.style.color = "red";
            return false;
        }
    ...
    var resetTrade = function (initiator, recipient, allowRecipientToBeChanged) {
        ...
        for (var i = 0; i < 40; i++) {
            ...
            if (currentSquare.owner === initiator.index) {
                ...
                currentTableCellCheckbox.title = "Marque esta caixa para vender " + currentSquare.name + "";
        ...
        if (initiator.communityChestJailCard) {
            ...
            currentTableCellCheckbox.title = "Marque esta caixa para vender o cartão Saia da Cadeia .";
            ...
            currentTableCell.textContent = "Cartao Saia da Cadeia";
        ...
        if (initiator.chanceJailCard) {
            ...
            currentTableCell.textContent = "Cartao Saida da Cadeia";
        ...
        if (initiatorSideTable.lastChild) {
            ...
            initiatorProperty.textContent = initiator.name + " nao tem propriedades para vender.";
        }

        if (recipientSideTable.lastChild) {
            recipientProperty.appendChild(recipientSideTable);
        } else {
            recipientProperty.textContent = recipient.name + " nao tem propriedades para vender.";
        }
        ...
        if (allowRecipientToBeChanged && pcount > 2) {
            ...
            nameSelect.title = "Selecione um jogador para negociar.";
        } else {
    ...
    this.acceptTrade = function (tradeObj) {
        if (isNaN(document.getElementById("trade-leftp-money").value)) {
            document.getElementById("trade-leftp-money").value = "Este valor deve ser um numero.";
            document.getElementById("trade-leftp-money").style.color = "red";
            return false;
        }

        if (isNaN(document.getElementById("trade-rightp-money").value)) {
            document.getElementById("trade-rightp-money").value = "Este valor deve ser um numero.";
        ...
        if (money > 0 && money > initiator.money) {
            document.getElementById("trade-leftp-money").value = initiator.name + " nao tem $" + money + ".";
            document.getElementById("trade-leftp-money").style.color = "red";
            return false;
        } else if (money < 0 && -money > recipient.money) {
            document.getElementById("trade-rightp-money").value = recipient.name + " nao tem $" + (-money) + ".";
            document.getElementById("trade-rightp-money").style.color = "red";
            return false;
        }
        ...
        if (showAlerts && !confirm(initiator.name + ", tem certeza de que deseja fazer essa troca com " + recipient.name + "?")) {
            return false;
        }
        ...
        if (tradeObj.getCommunityChestJailCard() === 1) {
            ...
            addAlert(recipient.name + ' recebeu um cartão "Saia da Cadeia" de ' + initiator.name + ".");
        } else if (tradeObj.getCommunityChestJailCard() === -1) {
            initiator.communityChestJailCard = true;
            recipient.communityChestJailCard = false;
            addAlert(initiator.name + ' recebeu um cartão "Saia da Cadeia" de ' + recipient.name + ".");
        }
        ...
        if (money > 0) {
            ...
            addAlert(recipient.name + " recebido $" + money + " de " + initiator.name + ".");
        } else if (money < 0) {
            money = -money;
            ...
            addAlert(initiator.name + " recebido $" + money + " de " + recipient.name + ".");
        }
    ...
    this.proposeTrade = function () {
        ...
        if (recipient.human) {
            ...
            addAlert(initiator.name + " iniciou uma negociacao com " + recipient.name + ".");
            popup("<p>" + initiator.name + " propos uma negociacao com voce, " + recipient.name + ". Você pode aceitar, rejeitar ou modificar a oferta.</p>");
        } else {
            ...
            if (tradeResponse === true) {
                ...
                popup("<p>" + recipient.name + " rejeitou sua oferta.</p>");
                return;
            } else if (tradeResponse instanceof Trade) {
                popup("<p>" + recipient.name + " propôs uma contra-proposta.</p>");
    ...
    this.eliminatePlayer = function () {
        ...
        if (pcount === 1) {
            ...
            popup("<p>Parabens, " + player[1].name + ", voce venceu o jogo.</p><div>");
    ...
    this.bankruptcyUnmortgage = function () {
        ...
        var HTML = "<p>" + player[p.creditor].name + ", você pode desipotecar de qualquer uma das seguintes propriedades, sem juros, clicando nelas. Clique em OK quando terminar.</p><table>";
        var price;
        ...
        for (var i = 0; i < 40; i++) {
            ...
            if (sq.owner == p.index && sq.mortgage) {
                ...
                // O jogador já pagou juros, para que eles possam cancelar a hipoteca pelo preço da hipoteca.
                HTML += "' onmouseover='showdeed(" + i + ");' onmouseout='hidedeed();'></td><td class='propertycellname'><a href='javascript:void(0);' title='Cancela Hipoteca " + sq.name + " por $" + price + ".' onclick='if (" + price + " <= player[" + p.creditor + "].money) {player[" + p.creditor + "].pay(" + price + ", 0); square[" + i + "].mortgage = false; addAlert(\"" + player[p.creditor].name + " cancelou hipoteca " + sq.name + " por $" + price + ".\");} this.parentElement.parentElement.style.display = \"none\";'>Cancelou hipoteca " + sq.name + " ($" + price + ")</a></td></tr>";
    ...
    this.resign = function () {
        popup("<p>Tem certeza de que deseja renunciar?</p>", game.bankruptcy, "Sim/Nao");
    };
    ...
    this.bankruptcy = function () {
        ...
        if (pcount === 2 || bankruptcyUnmortgageFee === 0 || p.creditor === 0) {
            ...
            addAlert(pcredit.name + " pago $" + bankruptcyUnmortgageFee + " juros sobre as propriedades hipotecadas recebidas de" + p.name + ".");
            popup("<p>" + pcredit.name + ", voce deve pagar $" + bankruptcyUnmortgageFee + " juros sobre as propriedades hipotecadas que você recebeu de " + p.name + ".</p>", function () {
...
function popup(HTML, action, option) {
    ...
    // Yes/No
    if (option === "Sim/Nao") {
        document.getElementById("popuptext").innerHTML += "<div><input type=\"button\" value=\"Sim\" id=\"popupyes\" /><input type=\"button\" value=\"Nao\" id=\"popupno\" /></div>";
...
function updateDice() {
    ...
    if (document.images) {
        ...
        element0.title = "Dado (" + die0 + " spots)";
        element1.title = "Dado (" + die1 + " spots)";
...
function updateOwned() {
        ...
        if (sq.owner == turn) {
            ...
            if (sq.house >= 1 && sq.house <= 4) {
                for (var x = 1; x <= sq.house; x++) {
                    housetext += "<img src='/static/images/house.png' alt='' title='Casa' class='house' />";
                }
            } else if (sq.hotel) {
    ...
    if (p.communityChestJailCard) {
        ...
        HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox40' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Cartao Saia da Cadeia</td></tr>";
    ...
    if (p.chanceJailCard) {
        ...
        HTML += "<tr class='property-cell-row'><td class='propertycellcheckbox'><input type='checkbox' id='propertycheckbox41' /></td><td class='propertycellcolor' style='background: white;'></td><td class='propertycellname'>Cartao Saia da Cadeia</td></tr>";
    }

    if (HTML === "") {
        HTML = p.name + ", voce nao tem nenhuma propriedade.";
        $("#option").hide();
    } else {
        HTML += "</table>";
    }
...
function updateOption() {
    ...
    if (checkedproperty < 0 || checkedproperty >= 40) {
        ...
        $("#buildings").show();
        document.getElementById("buildings").innerHTML = "<img src='/static/images/house.png' alt='' title='Casa' class='house' />:&nbsp;" + housesum + "&nbsp;&nbsp;<img src='/static/images/hotel.png' alt='' title='Hotel' class='hotel' />:&nbsp;" + hotelsum;
    ...
    if (sq.mortgage) {
        document.getElementById("mortgagebutton").value = "Desipotecar ($" + Math.round(sq.price * 0.6) + ")";
        document.getElementById("mortgagebutton").title = "Desipoteca " + sq.name + " por $" + Math.round(sq.price * 0.6) + ".";
        $("#buyhousebutton").hide();
        $("#sellhousebutton").hide();

        allGroupUnmortgaged = false;
    } else {
        document.getElementById("mortgagebutton").value = "Hipoteca ($" + (sq.price * 0.5) + ")";
        document.getElementById("mortgagebutton").title = "Hipotecado " + sq.name + " por $" + (sq.price * 0.5) + ".";
        ...
        if (sq.groupNumber >= 3) {
            ...
            buyhousebutton.value = "Comprar casa ($" + sq.houseprice + ")";
            sellhousebutton.value = "Vender casa ($" + (sq.houseprice * 0.5) + ")";
            buyhousebutton.title = "Comprar uma casa por $" + sq.houseprice;
            sellhousebutton.title = "Vender uma casa por $" + (sq.houseprice * 0.5);

            if (sq.house == 4) {
                buyhousebutton.value = "Comprar hotel ($" + sq.houseprice + ")";
                buyhousebutton.title = "Comprar um hotel por $" + sq.houseprice;
            }
            if (sq.hotel == 1) {
                $("#buyhousebutton").hide();
                sellhousebutton.value = "Vender hotel ($" + (sq.houseprice * 0.5) + ")";
                sellhousebutton.title = "Vender um hotel por $" + (sq.houseprice * 0.5);
            }
            ...
            for (var i = 0; i < max; i++) {
                ...
                if (s.owner !== sq.owner) {
                    ...
                    buyhousebutton.title = "Antes de comprar uma casa, você deve possuir todas as propriedades desse grupo de cores.";
                } else {
            ...
            if (!allGroupUnmortgaged) {
                buyhousebutton.disabled = true;
                buyhousebutton.title = "Antes de comprar uma casa, você deve desipotecar de todas as propriedades desse grupo de cores.";
            }
            ...
            if (sq.house > minhouse) {
                ...
                if (sq.house == 1) {
                    buyhousebutton.title = "Antes que você possa comprar outra casa, todas as outras propriedades desse grupo de cores devem ter uma casa.";
                } else if (sq.house == 4) {
                    buyhousebutton.title = "Antes de comprar um hotel, todas as outras propriedades desse grupo de cores devem ter 4 casas.";
                } else {
                    buyhousebutton.title = "Antes de comprar uma casa, todas as outras propriedades desse grupo de cores devem ter " + sq.house + " casas.";
                }
            }
            if (sq.house < maxhouse) {
                ...
                if (sq.house == 1) {
                    sellhousebutton.title = "Antes que você possa vender uma casa, todas as outras propriedades desse grupo de cores devem ter uma casa.";
                } else {
                    sellhousebutton.title = "Antes que você possa vender uma casa, todas as outras propriedades desse grupo de cores devem ter" + sq.house + " casas.";
                }
            }
            ...
            if (!allGroupUninproved) {
                document.getElementById("mortgagebutton").title = "Antes que uma propriedade possa ser hipotecada, todas as propriedades do seu grupo de cores devem ser melhoradas.";
...
function chanceCommunityChest() {
    ...
    if (p.position === 2 || p.position === 17 || p.position === 33) {
        ...
        popup("<img src='/static/images/community_chest_icon.png' style='height: 50px; width: 53px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Cofre:</div><div style='text-align: justify;'>" + communityChestCards[communityChestIndex].text + "</div>", function () {
        ...
        popup("<img src='/static/images/chance_icon.png' style='height: 50px; width: 26px; float: left; margin: 8px 8px 8px 0px;' /><div style='font-weight: bold; font-size: 16px; '>Sorte:</div><div style='text-align: justify;'>" + chanceCards[chanceIndex].text + "</div>", function () {
...
function gotojail() {
    ...
    document.getElementById("nextbutton").value = "Fim de Turno";
    document.getElementById("nextbutton").title = "Finalize o turno e avance para o próximo jogador.";
...
function advance(destination, pass) {
    ...
    if (typeof pass === "number") {
        if (p.position < pass) {
            ...
            p.position = pass;
            p.money += 200;
            addAlert(p.name + " coletou um salário de US $ 200 por passar pelo Inicio.");
        }
    }
...
function streetrepairs(houseprice, hotelprice) {
    ...
    if (cost > 0) {
        ...
        if (houseprice === 40) {
            addAlert(p.name + " perdeu $" + cost + " para o Cofre.");
        } else {
            addAlert(p.name + " perdeu $" + cost + " para a Sorte.");
        }
    }
...
function payfifty() {
    ...
    addAlert(p.name + " pagou a multa de US $ 50 para sair da prisão.");
    updateMoney();
    updatePosition();
}
...
function useJailCard() {
    ...
    addAlert(p.name + " usou um cartão \"Saia da Cadeia\".");
    updateOwned();
    updatePosition();
}
...
function showStats() {
    ...
    for (var x = 1; x <= pcount; x++) {
        ...
        for (var i = 0; i < 40; i++) {
            ...
            if (sq.owner == x) {
                ...
                if (sq.house == 5) {
                    ...
                    housetext += "<span style='float: right; font-weight: bold;'>" + sq.house + "&nbsp;x&nbsp;<img src='/static/images/house.png' alt='' title='Casa' class='house' style='float: none;' /></span>";
                }
        ...
        if (p.communityChestJailCard) {
            ...
            HTML += "<tr><td class='statscellcolor'></td><td class='statscellname'>Cartao Saia da Cadeia</td></tr>";
        ...
        if (!write) {
            HTML += p.name + " nao possui propriedades.";
        } else {
            HTML += "</table>";
        }
...
function buy() {
    ...
    if (p.money >= cost) {
        ...
        popup("<p>" + p.name + ", voce precisa de mais $" + (property.price - p.money) + " para comprar " + property.name + ".</p>");
    }
}
...
function mortgage(index) {
    ...
    document.getElementById("mortgagebutton").value = "Cancela hipoteca por $" + unmortgagePrice;
    document.getElementById("mortgagebutton").title = "Cancela hipoteca " + sq.name + " por $" + unmortgagePrice + ".";

    addAlert(p.name + " hipotecado " + sq.name + " por $" + mortgagePrice + ".");
    updateOwned();
    updateMoney();
...
function unmortgage(index) {
    ...
    sq.mortgage = false;
    document.getElementById("mortgagebutton").value = "Hipoteca por $" + mortgagePrice;
    document.getElementById("mortgagebutton").title = "Hipoteca " + sq.name + " por $" + mortgagePrice + ".";

    addAlert(p.name + " desipotecou " + sq.name + " por $" + unmortgagePrice + ".");
    updateOwned();
    return true;
}
...
function land(increasedRent) {
    ...
    $("#landed").show();
    document.getElementById("landed").innerHTML = "Você pousou em " + s.name + ".";
    s.landcount++;
    addAlert(p.name + " pousou em " + s.name + ".");
    ...
    if (s.price !== 0 && s.owner === 0) {
        ...
        if (!p.human) {
            ...
            document.getElementById("landed").innerHTML = "<div>Voce pousou em <a href='javascript:void(0);' onmouseover='showdeed(" + p.position + ");' onmouseout='hidedeed();' class='statscellcolor'>" + s.name + "</a>.<input type='button' onclick='buy();' value='Comprar ($" + s.price + ")' title='Comprar " + s.name + " por " + s.pricetext + ".'/></div>";
        }
    ...
    if (s.owner !== 0 && s.owner != turn && !s.mortgage) {
        ...
        document.getElementById("landed").innerHTML = "Voce pousou em " + s.name + ". " + player[s.owner].name + " receba $" + rent + " aluguel.";
    } else if (s.owner > 0 && s.owner != turn && s.mortgage) {
        document.getElementById("landed").innerHTML = "Voce pousou em " + s.name + ". A propriedade está hipotecada; nenhum aluguel foi cobrado.";
    }
    ...
    if (p.position === 30) {
        ...
        if (p.human) {
            popup("<div>Vá para a cadeia. Vá diretamente para a cadeia. Não passe pelo INICIO. Não colete US $ 200.</div>", gotojail);
        } else {
            gotojail();
        }
...
function roll() {
    ...
    if (die1 == die2) {
        addAlert(p.name + " tirou " + (die1 + die2) + " - dados duplicados.");
    } else {
    ...
    if (die1 == die2 && !p.jail) {
        ...
        if (doublecount < 3) {
            document.getElementById("nextbutton").value = "Jogue novamente";
            document.getElementById("nextbutton").title = "Voce fez duplas. Jogue novamente.";

            // If player rolls doubles three times in a row, send him to jail
        } else if (doublecount === 3) {
            p.jail = true;
            doublecount = 0;
            addAlert(p.name + " jogou dados duplicados 3 vezes seguidas.");
            updateMoney();


            if (p.human) {
                popup("Você jogou dados duplicados três vezes seguidas. Vá para a cadeia.", gotojail);
            } else {
                gotojail();
            }
    ...
    if (p.jail === true) {
        ...
        if (die1 == die2) {
            ...
            addAlert(p.name + " jogue dados duplicados para sair da cadeia.");
            ...
            if (p.jailroll === 3) {
                ...
                if (p.human) {
                    popup("<p>Você deve pagar a multa de US $ 50.</p>", function () {
                        payfifty();
                ...
                $("#landed").show();
                document.getElementById("landed").innerHTML = "Você está preso.";
...
function play() {
    ...
    document.getElementById("nextbutton").value = "Jogue os Dados";
    document.getElementById("nextbutton").title = "Lance os dados e mova seu token de acordo.";
    ...
    if (p.jail) {
        $("#landed").show();
        document.getElementById("landed").innerHTML = "Voce esta na cadeia.<input type='button' title='Pague multa de $ 50 para sair da cadeia imediatamente.' value='Pagar multa de $ 50' onclick='payfifty();' />";

        if (p.communityChestJailCard || p.chanceJailCard) {
            document.getElementById("landed").innerHTML += "<input type='button' id='gojfbutton' title='Usar &quot;Saia da Cadeia&quot; card.' onclick='useJailCard();' value='Usar Cartao' />";
        }

        document.getElementById("nextbutton").title = "Lance os dados. Se você conseguir dados duplicados, você sairá da cadeia.";

        if (p.jailroll === 0)
            addAlert("Esta é a primeira vez de " + p.name + "' na cadeia.");
        else if (p.jailroll === 1)
            addAlert("Esta é a segunda vez de " + p.name + "' na cadeia.");
        else if (p.jailroll === 2) {
            document.getElementById("landed").innerHTML += "<div>NOTA: Se você não jogar duplas após esta jogada, voce <i>deve</i> pagar a multa de $ 50.</div>";
            addAlert("Esta é a terceira vez de " + p.name + "' na cadeia.");
        }
...
window.onload = function () {
    ...
    document.getElementById("enlarge-wrap").innerHTML += "<div id='enlarge40' class='enlarge'><div id='enlarge40color' class='enlarge-color'></div><br /><div id='enlarge40name' class='enlarge-name'>Cadeia</div><br /><div id='enlarge40price' class='enlarge-price'><img src='/static/images/jake_icon.png' height='80' width='80' alt='' style='position: relative; top: -20px;' /></div><br /><div id='enlarge40token' class='enlarge-token'></div></div>";
    ...
    $("#mortgagebutton").click(function () {
        ...
        if (s.mortgage) {
            if (player[s.owner].money < Math.round(s.price * 0.6)) {
                popup("<p>Voce precisa de $" + (Math.round(s.price * 0.6) - player[s.owner].money) + " a mais para desipotecar de " + s.name + ".</p>");

            } else {
                popup("<p>" + player[s.owner].name + ", tem certeza de que deseja desipotecar " + s.name + " for $" + Math.round(s.price * 0.6) + "?</p>", function () {
                    unmortgage(checkedProperty);
                }, "Sim/Nao");
            }
        } else {
            popup("<p>" + player[s.owner].name + ", tem certeza de que deseja hipotecar " + s.name + " for $" + Math.round(s.price * 0.5) + "?</p>", function () {
    ...
    $("#buyhousebutton").on("click", function () {
        ...
        if (p.money < s.houseprice) {
            if (s.house === 4) {
                popup("<p>Voce precisa de $" + (s.houseprice - player[s.owner].money) + " a mais para comprar um hotel para " + s.name + ".</p>");
                return;
            } else {
                popup("<p>Voce precida de $" + (s.houseprice - player[s.owner].money) + " a mais para comprar uma casa para " + s.name + ".</p>");
                return;
            }
        }
        ...
        if (s.house < 4 && houseSum >= 32) {
            popup("<p>Todas as 32 casas são de propriedade. Você deve esperar até que um fique disponível.</p>");
            return;
        } else if (s.house === 4 && hotelSum >= 12) {
            popup("<p>Todos os 12 hotéis são de propriedade. Você deve esperar até que um fique disponível.</p>");
            return;
        }
